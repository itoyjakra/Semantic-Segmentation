...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    """
    Creates the layers for a fully convolutional network.  Builds skip-layers using the vgg layers.
    ...
    vgg_74_up2 = tf.layers.conv2d_transpose(vgg_74, num_classes, 4, strides=(2, 2), padding='same')
    vgg_743 = tf.add(vgg_74_up2, vgg3_conv1)

    output = tf.layers.conv2d_transpose(vgg_743, num_classes, 16, strides=(8, 8), padding='same')
...
def run():
    ...
    with tf.Session() as sess:
        ...
        get_batches_fn = helper.gen_batch_function_aug(os.path.join(data_dir, 'data_road/training'), image_shape)
        ...
        train_nn(sess, max_epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, image_input, correct_label, keep_prob, learning_rate)
